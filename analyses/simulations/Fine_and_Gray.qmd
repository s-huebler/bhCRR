...
p5_ssl_minPenalty <- fit_ssl_psdh(p5$x,p5$y, ss = c(0.001, 1),
                            ...
                            fixed_global_shrinkage  = p5_fstcmp_minPenalty$lambda.path)
```
...
psdh_ssl_func <- function(x, y, init_sparsity, ncv_count,
  ...
  x <- as.matrix(x)
  y <- as.matrix(y)
  fit <- fit_ssl_psdh(x, y, initial_sparsity = init_sparsity, fixed_global_shrinkage = fixed_global_shrinkage)
...
```{r}
viz_comparison(p10_comparisons, viztype = "full")
viz_comparison(p10_comparisons, viztype = "difference")
```
