...
```{r}
psdh_ssl_func <- function(x,y, init_sparsity){
  x <- as.matrix(x)
  y <- as.matrix(y)
...
ssl_mod <- psdh_ssl_func(easy_sim_data %>%
                         ...
                         easy_sim_data %>%
                           ...
                           select(TTE, Status), init_sparsity = 0.999)
```
...
# Run it for a few of the (s0, s1) pairs where you saw identical means
debug_cv(0.005, 0.5)
debug_cv(0.05,  0.5)
debug_cv(0.1,   0.5)
```
