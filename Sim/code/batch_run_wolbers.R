...
npredictors_grid <- c(30)      # set of npredictors to sweep over
...
run_once <- function(nobs, npredictors, beta1_active, beta2_active,
  ...
  list(
    ...
    final_mod = final_mod,
    fg_true   = fg_true,
    # Wolbers-selected comparators (LASSO / aLASSO / SCAD / MCP). Each element is the
    # select_by_wolbers() summary, incl. beta_raw (original units) and beta_std.
